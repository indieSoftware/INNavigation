...
public class Router: ObservableObject, Sendable {
	...
	public func present(_ route: Route, type: PresentationType = .sheet(detents: [.large])) {
		Task {
	...
	public nonisolated func present(_ route: Route, type: PresentationType = .sheet(detents: [.large])) async {
		await transition(direction: .vertical) {
			applyPresent(route, type: type)
		}
	}

	func applyPresent(_ route: Route, type: PresentationType = .sheet(detents: [.large])) {
