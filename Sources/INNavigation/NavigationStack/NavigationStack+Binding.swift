...
public extension Binding {
	...
	func present<Page>(
		...
		#if !os(macOS)
			wrappedValue.pageElements.append(NavigationPageElement<Page>(page: page, options: options))
			DispatchQueue.main.asyncAfter(deadline: .now() + .milliseconds(wrappedValue.pushPresentMilliseconds)) {
				completion?()
			}
		#else
			if options.style != .sheet {
				pop()
			}
			wrappedValue.pageElements.append(NavigationPageElement<Page>(page: page, options: options))
			DispatchQueue.main.asyncAfter(deadline: .now() + .milliseconds(wrappedValue.pushPresentMilliseconds)) {
	...
	func pop<Page>(
		...
		guard wrappedValue.pageElements.count > 1 else {
			completion()
			return
		}
		wrappedValue.pageElements = wrappedValue.pageElements.dropLast()
